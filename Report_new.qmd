...
For this project, Derek calculate the liquidity for the data clustering, Alan did the k-means clustering, and each group member trains a model for comparing. After every one got the results, Derek generate the overall MSE boxplot, and Yuxiang integrated all the code and calculated the training time for each model.

For this report, Baiyu responsible for Aim & background part. Isabella did the Method B. Derek and Tom completed the Results A part. Yuxiang responsible for Results B, and Discussion & Conclusion wrote by Alan. Everyone in the group wrote their own model description part in Method A.
