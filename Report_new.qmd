...
Our group put the GitHub link into Appendix.
...
For this project, Derek calculated the liquidity for the data clustering, Alan did the k-means clustering, and each group member trained a model for comparing. After every one got the results, Derek generated the overall RMSE boxplot, and Yuxiang integrated all the code and calculated the training time for each model. Alan and Isabella did the shiny app.

For this report, Baiyu is responsible for Aim & Background part. Isabella did the Method Part B. Derek and Tom completed the Results Part A. Yuxiang responsible for Results Part B, and Discussion & Conclusion is written by Alan. Everyone in the group wrote their own model description part in Method A.
...
1.  Boxplots for each cluster

![Cluster1](figure1.png) ![Cluster2](figure1.png) ![Cluster3](figure1.png) ![Cluster4](figure1.png) ![Cluster5](figure1.png) ![Cluster6](figure1.png)

2.  Here is the coding part for clustering stocks, stock selection, process of SVM model and production of RMSE box plots.
...
for(i in 1 : length(stock_file1)){
  ...
  for (a in 1 : length(sample_time_id)) {
    sec <- temporary %>% filter(time_id == sample_time_id[a]) %>% pull(seconds_in_bucket)
...
for(i in 1 : length(stock_file3)){
  ...
  for (a in 1 : length(sample_time_id)) {
    sec <- temporary %>% filter(time_id == sample_time_id[a]) %>% pull(seconds_in_bucket) #对于sample_time_id 
    BS1 <- temporary %>% filter(time_id == sample_time_id[a]) %>% pull(BidAskSpread1)
    #BS2 <- temporary %>% filter(time_id == sample_time_id[a]) %>% pull(BidAskSpread2)
    order <- temporary %>% filter(time_id == sample_time_id[a]) %>% pull(num_order)
    price <- temporary %>% filter(time_id == sample_time_id[a]) %>% pull(WAP) 
...
https://alanx.shinyapps.io/Optiver_13_Shiny/

9.  GitHub Link

    https://github.com/isabelllag/data3888report.git
